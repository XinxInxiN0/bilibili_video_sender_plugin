...
class BilibiliParser:
    ...
    def _select_video_stream(
        ...
        if target_qn > 0:
            if strict_qn:
                eligible = [video for video in videos if BilibiliParser._safe_int(video.get("id")) == target_qn]
                if not eligible:
        ...
        if strict_qn and target_qn > 0:
            ...
            if best_id > 0:
                candidates = [video for video in eligible if BilibiliParser._safe_int(video.get("id")) == best_id]
            else:
                candidates = list(eligible)
    ...
    def get_view_info_by_url(
        ...
        if page_index > len(pages):
            BilibiliParser._logger.warning(f"分P参数超出范围: p={page_index}, total={len(pages)}，回退到 P1")
            page_index = 1
    ...
    def get_play_urls(
        ...
        if not dash:
            ...
            if durl:
                ...
                if len(durl) > 1:
                    BilibiliParser._logger.warning(f"durl为多段视频（{len(durl)}段），当前仅处理第一段")
                item = durl[0]
        ...
        if selected_qn is not None:
            ...
            if requested_qn != 0 and selected_qn != requested_qn:
                ...
                BilibiliParser._logger.info(f"Quality selected: requested_qn={requested_qn}, selected_qn={selected_qn}")

        if selection_status == "fallback":
            BilibiliParser._logger.info(f"No eligible streams for qn={qn}, fell back to best available stream")
                ...
                async with session.post(api_url, json=request_data, headers=headers, timeout=300) as response:
                    ...
                    if response.status in (401, 403) and token:
                        ...
                        async with session.post(
                            retry_url, json=request_data, headers=headers, timeout=300
                        ) as retry_response:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(api_url, json=request_data, headers=headers, timeout=300) as response:
                    ...
                    if response.status in (401, 403) and token:
                        error_text = await response.text()
                        self._logger.warning(f"Group video auth failed ({response.status}), retrying with access_token")
                        retry_url = f"{api_url}?access_token={urllib.parse.quote(token)}"
                        async with session.post(
                            retry_url, json=request_data, headers=headers, timeout=300
                        ) as retry_response:
                ...
                def _blocking() -> Optional[Tuple[BilibiliVideoInfo, Dict[str, Any], str]]:
                    ...
                    if "b23.tv" in target_url:
                        ...
                        for attempt in range(max_retries):
                            try:
                                self._logger.debug(
                                    f"Resolving short link (attempt {attempt + 1}/{max_retries}): {target_url}"
                                )
                                ...
                                self._logger.warning(
                                    f"Failed to resolve short link (attempt {attempt + 1}/{max_retries}): {e}"
                                )
                                if attempt < max_retries - 1:
                                    ...
                                    self._logger.error(
                                        f"Failed to resolve short link after {max_retries} attempts, using original URL"
                                    )
                    ...
                    for attempt in range(max_retries):
                        try:
                            self._logger.debug(
                                f"Parsing video info (attempt {attempt + 1}/{max_retries}): {target_url}"
                            )
                            ...
                            if info:
                                ...
                                self._logger.warning(
                                    f"Failed to parse video info (attempt {attempt + 1}/{max_retries}): returned None"
                                )
                        except Exception as e:
                            self._logger.warning(
                                f"Failed to parse video info (attempt {attempt + 1}/{max_retries}): {e}"
                            )
                    ...
                    self._logger.debug(
                        "Playback sources fetched", status=status, source_type=source_type, title=info.title
                    )
    ...
    config_schema: Dict[str, Dict[str, ConfigField]] = {
        "plugin": {
            ...
            "use_new_events_manager": ConfigField(
                type=bool,
                default=True,
                description="是否使用新版 events_manager（0.10.2 及以上版本设为 True，否则设为 False）",
            ),
        },
        "bilibili": {
            "sessdata": ConfigField(
                type=str, default="", description="B 站登录 Cookie 中的 SESSDATA 值（用于获取高清晰度视频）"
            ),
            "buvid3": ConfigField(
                type=str, default="", description="B 站设备标识 Buvid3（可选，用于生成 session 参数）"
            ),
            "qn": ConfigField(
                type=int,
                default=0,
                description="清晰度设置(qn)，0 为自动（登录默认 720P，未登录默认 480P）。常见值：16 = 360P, 32 = 480P, 64 = 720P, 74 = 720P60, 80 = 1080P, 112 = 1080P+, 116 = 1080P60, 120 = 4K, 125 = HDR, 126 = 杜比视界, 127 = 8K",
            ),
            "qn_strict": ConfigField(
                type=bool,
                default=False,
                description="是否严格按 qn 选择清晰度。False 时会在可用流中自动降级/回退；True 时不可用则报错",
            ),
            "group_at_only": ConfigField(type=bool, default=False, description="群聊中仅当被 @ 时才处理 B 站链接"),
            "block_ai_reply": ConfigField(
                type=bool,
                default=True,
                description="检测到 B 站视频链接后是否阻止后续 AI 回复（仅影响本次事件链路）（旧版 events_manager 下可能无效）",
            ),
            "store_plugin_text": ConfigField(
                type=bool, default=False, description="插件发送的文本消息是否写入历史记录（False 则不入库）"
            ),
            "enable_video_compression": ConfigField(type=bool, default=True, description="是否启用视频压缩功能"),
            "max_video_size_mb": ConfigField(
                type=int, default=100, description="视频文件大小限制（MB），超过此大小将进行压缩"
            ),
            "compression_quality": ConfigField(
                type=int, default=23, description="视频压缩质量 (1-51，数值越小质量越高，推荐 18-28)"
            ),
            "enable_duration_limit": ConfigField(type=bool, default=True, description="是否启用视频时长限制"),
            "max_video_duration": ConfigField(
                type=int, default=600, description="视频最大时长限制（秒），超过此时长将拒绝发送"
            ),
        },
        "parser": {
